...
func NewMultiTree(logger logger.Logger, rootPath string, opts itree.Options) *MultiTree {
	...
	return &MultiTree{
		Trees:    make(map[string]*itree.Tree),
...
func (mt *MultiTree) MountTrees() error {
	paths, err := comutil.FindDbsInPath(mt.rootPath)
	if err != nil {
		return err
	}
	for _, dbPath := range paths {
...
func (mt *MultiTree) newTree(dbPath string) (*itree.Tree, error) {
	pool := nodepool.NewNodePool()
	opts := mt.treeOpts
	if _, ok := mt.treeOpts.MetricsProxy.(*metrics.StructMetrics); ok {
		opts.MetricsProxy = metrics.NewStructMetrics()
	}
	sql, err := sqlite.NewDB(sqlite.DefaultOptions(sqlite.Options{
		Path:    dbPath,
	...
	return itree.NewTree(sql, pool, opts), nil
}
...
func (mt *MultiTree) QueryReport(bins int) error {
	...
	for _, tree := range mt.Trees {
		sm, ok := tree.Metrics().(*metrics.StructMetrics)
		if !ok {
			continue
		}
		m.Add(sm)
...
func (mt *MultiTree) TestBuild(opts *testutil.TreeBuildOptions) (int64, error) {
	...
	report := func() error {
		...
		for _, tr := range mt.Trees {
			sm, ok := tr.Metrics().(*metrics.StructMetrics)
			if !ok {
				return nil
			}
	...
	for ; itr.Valid(); err = itr.Next() {
		...
		changesetByStore := make(map[string][]itree.BatchOperation)
		changeset := itr.Nodes()
		for ; changeset.Valid(); err = changeset.Next() {
			...
			var op itree.BatchOperation
			if !node.Delete {
				op = itree.NewSetOperation(key, node.Value)
			} else {
				op = itree.NewRemoveOperation(key)
			}
